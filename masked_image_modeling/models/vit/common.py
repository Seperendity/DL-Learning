...
class ViTBlock(nn.Moudle):
    def __init__(self,
                 dim        :int,
                 num_heads  :int,
                 mlp_ratio  :float = 4.0,
                 qkv_bias   :bool = True,
                 act_layer  :Type[nn.Moudle] = nn.GELU,
                 dropout    :float = 0.
                 ) -> None:
        super().__init__()
        ...
        self.ffn = FeedForward(embedding_dim=dim, mlp_dim= dim * mlp_ratio, act=act_layer)
...
class CrossAttention(nn.Moudle):
    def __init__(self,
                 in_dim     : int,
                 out_dim    : int,
                 qkv_bias   : bool,
        ...
        self.cls_token = nn.parameters(torch.randn(1, num_queries, in_dim) * 0.02)
