...
class MaeEncoder(nn.Moudle):
    def __init__(self,
        ...
        self.pos_embed = nn.Parameter(nn.zeros(1, self.num_patches, patch_embed_dim), requires_grad= False)
    ...
    def random_masking(x, mask_ratio= 0.75):
        B, N, C = x.shape
        len_keep = int(N * (1 - mask_ratio)) # the number of retained token
...
class MaeDecoder(nn.Module):
    ...
    def forward(self, x_enc, ids_restore):
        ...
        mask_tokens = self.mask_token.repeat(B, ids_restore.shape[1] - N_nomask, 1)
        x_all = torch.cat([x_enc, mask_tokens], dim=1)

        x_all = torch.gather(x_all, dim=1, index = ids_restore.unsqueeze(-1).repeat(1, 1, C))
...
# ------------------------ Model Functions ------------------------
def build_vit_mae(model_name="vit_t", img_size=224, patch_size=16, img_dim=3, mask_ratio=0.75, norm_pix_loss=False):
    # ---------------- MAE Encoder ----------------
    if model_name == "vit_t":
